<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:82:"/home/wwwroot/www.yuanzigo.com/public/../application/mobile/view/passport/reg.html";i:1524460678;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
...
<div id="ckdiv" style="">
	<div class="text-center mar-btm">
		<img id="headimg" src="<?php echo (isset($weixin['headimgurl']) && ($weixin['headimgurl'] !== '')?$weixin['headimgurl']:'http://www.isok.cc/html/nifty/img/profile-photos/2.png'); ?>" class="img-lg img-circle mar-top">
		<p class="text-bold pad-top text-lg"><span id="nickname"><?php echo $weixin['nickname']; ?></span>的小店</p>
	</div>
...
<div id="basediv" style="display:none">
	<div class="bg-light pad-top">
		<div class="form-horizontal">
			...
			<span class="tags" data-id="<?php echo $v['id']; ?>"><?php echo $v['name']; ?></span>
...
<script>
base_url='http://www.xiaomange.com/api4/'
	$('.tags').click(function(){
		var that=$(this);
		if(that.hasClass('on')){
			that.removeClass('on')
	...
	function reg(){	
		if(!$('#read-radio').is(':checked')){
			layer.alert('请先阅读《原子出发平台服务协议》');
			return false;
		}		
		...
		var ycode=$("#regbtn").attr('data-code')
		if(yzm==ycode){
			$('#ckdiv').hide()
			$('#basediv').show()
		}else{
			...
			$.post(getYzmUrl,{mobile:mobile},function(data){
				...
				if(data.code!=0){
					layer.msg(data.message)
						$('#regbtn').attr('data-code',data.data.code)
						var wait = 60;
	...
	function regshop(){
		...
		var mobile=$('#tel').val();
		var reg_url="<?php echo url('passport/regShop'); ?>"
		$.post(reg_url,{tags:tags,city:city,mobile:mobile},function(data){
			if(data.status==1){
				layer.alert('注册成功，开始【发现】优质的活动吧',function(){
					location.href="<?php echo url('mobile/activity/index'); ?>"
				})				
			}else{
